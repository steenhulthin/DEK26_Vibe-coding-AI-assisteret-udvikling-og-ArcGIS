...
108. Rettede blankt dashboard-sidepanel ved at bytte layout-orienteringer efter fungerende JSON: root left/right split bruger nu `orientation="col"`, mens højre panel stakker widgets lodret med `orientation="row"`; noterede at Dashboards kræver date selector for tidsfilter frem for automatisk web map time slider.
109. Undersøgte om Map Viewer time slider kan overføres til ArcGIS Dashboards; fandt ingen native Dashboard map-tool support og anbefalede alternativer: date selector/actions, måned/category selector, URL date parameters eller embedded/linket Instant App/custom Maps SDK app med rigtig TimeSlider.
